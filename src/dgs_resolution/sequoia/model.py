...
yamlpath = os.path.join(here, "sequoia-06212019.yaml")


def res_vs_E(E, chopper="", chopper_freq=600.0, Ei=100.0):
    instrument = PyChop2(yamlpath, chopper, chopper_freq)
    res, flux = instrument.getResFlux(Etrans=E, Ei_in=Ei)
    return res


def elastic_res_flux(chopper="SEQUOIA-100-1.5-SMI", chopper_freq=600.0, Ei=100.0):
...
def elastic_resolution(chopper="SEQUOIA-100-1.5-SMI", chopper_freq=600.0, Ei=100.0):
    instrument = _getModel(yamlpath, chopper, chopper_freq)
    try:
        return instrument.getResolution(Etrans=0.0, Ei_in=Ei)[0]
    except:
        return 0.0
