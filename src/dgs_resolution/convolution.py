...
class WidgetFactory:
    ...
    * update UI elements (to be used in callback functions)
    ...
    def createIQEInterface(self, app):
        ...
        return html.Div(
            [
                ...
                convolution_panel(
                    self.phonopy_upload_widget_id,
                    self.IQE_plot_widget_Id,
                    "DFT force constants zip file, or SQE hdf5 file",
                ),
                html.Div(),
                ...
                html.A(
                    "graphite S(Q,E) for Ei=300.meV, T=300K computed from DFT Force constants",
    ...
    def createPlotForUploadedData(self, uploaded_contents, uploaded_filename, uploaded_last_modified, Ei, *args):
        ...
        if len(E) > 1000:
            return [html.P("Too many data points: %s" % len(E), style={"color": "red", "fontSize": 12})]
        mask = E < Ei * 0.9
        E1 = E[mask]
        I1 = I[mask]
        ...
        curve = {
            ...
            "layout": {
                "title": "I(E) curve",
                "xaxis": {"title": "E (meV)"},
                "yaxis": {"title": "Intensity (arb. unit)"},
            },
        }
    ...
    def createExamplesSkeleton(self, app):
        ...
        example_panel = html.Div(
            id=self.conv_example_id, children=html.Div([self.createExcitationPanelSkeleton(), dcc.Loading(plots)])
        )
        return html.Details(
            [
    ...
    def createExcitationPanelSkeleton(self):
        ...
        inputarea = dcc.Textarea(
            id=self.excitation_input_id, value="", style={"width": "30em", "height": "10em", "margin-top": "1em"}
        )
        inputarea_container = html.Div([inputarea], style={"display": "inline-flex"})
        status = html.Div(id=self.excitation_input_status_id, children=[], style=dict(padding="1em", color="red"))
        apply_button = html.Button("Apply", id=self.apply_excitations_button_id)
        right = html.Div(children=[apply_button, status], style=dict(display="inline-flex", margin="1em"))
        return html.Details(
            [
    ...
    def updateExamplePanel(self, excitations_text, Ei, *args):
        ...
        * *args: additional args for convolution
        ...
        if not excitations_text.strip():
            excitations = np.linspace(-0.45 * Ei, Ei * 0.8, 8), np.ones(8)
            ...
            try:
                ...
                error = True
        if not error:
            E, I = IE_from_excitations(excitations, -0.5 * Ei, Ei * 0.95, 100)
    ...
    def updateSQEConvolution(self, uploaded_contents, uploaded_filename, qgrid_dim, Nqsamples, temperature, Ei, *args):
        if uploaded_contents is None:
            return
        ...
        if binfile.endswith(".zip"):
            zipfile = binfile
            # compute sqe
            Eaxis = np.linspace(-0.0 * Ei, 0.9 * Ei, 120)
            ...
            print(temperature)
            sqe = SQE_from_FCzip(
                Qaxis, Eaxis, zipfile, Ei, max_det_angle=140.0, T=temperature, qgrid_dim=qgrid_dim, Nqpoints=Nqsamples
            )
            os.unlink(zipfile)
        ...
        fig = go.Figure(
            data=[go.Heatmap(z=z, x=sqe.Q, y=sqe.E, zmin=0.0, zmax=zmedian * 5, colorscale="Viridis")],
            layout={
                "title": "Original",
            },
        )
        ...
        zmedian = np.median(z[z > 0])
        fig2 = go.Figure(
            data=[go.Heatmap(z=z, x=Q, y=E_new, zmin=0.0, zmax=zmedian * 5, colorscale="Viridis")],
            layout={
                "title": "Convolved",
            },
        )
    ...
    def convolveSQE(self, IQE, Ei, *args):
        max_det_angle = 140.0 * np.pi / 180.0
...
def makePSF(a, E, Ei):
    ...
    N = len(E_new)
    psf = np.zeros((N, N))
    from dgs_resolution import asymm
...
def gaussian(x, sigma):
    sigma2 = sigma * sigma
    return 1.0 / np.sqrt(2.0 * np.pi) / sigma * np.exp(-x * x / 2 / sigma2)
