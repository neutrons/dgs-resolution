...
import dgs_resolution.widget_utils as wu
...
from dgs_resolution.convolution import WidgetFactory as ConvolutionWF
...
conv_widget_factory = ConvolutionWF(
    instrument="arcs", instrument_params=instrument_params, res_function_calculator=get_data
)
        ...
        children=[
            ...
            html.Div(
                [
                    ...
                    # calculate button
                    html.Div(
                        [html.Button("Calculate", id="arcs-calculate-button")],
            ...
            if output_tab in ["summary", "plot"]:
                ...
                if conv_tab == "I(E)":
                    example_panel_excitation_placeholder, excitation_input_status, example_panel_plots = (
                        conv_widget_factory.updateExamplePanel(excitation_input_text, Ei, chopper_select, chopper_freq)
                    )
...
def update_summary_and_downloadlink(Ei, chopper_select, chopper_freq):
    downloadlink = "/download/arcs?chopper_select=%s&chopper_freq=%s&Ei=%s" % (chopper_select, chopper_freq, Ei)
    ...
    summary = summary_format_str.format(
        el_res=elastic_res, el_res_percentage=elastic_res / Ei * 100.0, Ei=Ei, flux=flux
    )
