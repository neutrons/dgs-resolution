...
from dgs_resolution.pychop import Chop, MulpyRep
...
def _check_input(self, *args):
    ...
    if [v for v in retval if v is None]:
        raise ValueError(
            "%s has not been specified." % (vtype[[i for i in range(len(retval)) if retval[i] is None][0]])
        )
...
class ChopperSystem(object):
    ...
    def _parse_choppers(self):
        ...
        for idx, chopper in enumerate(self.choppers):
            ...
            if "packages" in chopper:
                ...
                self.phaseNames.append(
                    chopper["phaseName"] if "phaseName" in chopper else "Chopper %d phase delay time" % (idx)
                )
        ...
        source_rep = (
            self.source_rep if (not hasattr(self, "n_frame") or self.n_frame == 1) else [self.source_rep, self.n_frame]
        )
        self._instpar = [
            self.distance,
            self.nslot,
    ...
    def plotMultiRepFrame(self, h_plt=None, Ei_in=None, frequency=None):
        ...
        for i in range(len(lines)):
            ...
            newline = [
                lineM * np.sqrt(1 + self.overlap_ei_frac),
                modSamDist - lineM * np.sqrt(1 + self.overlap_ei_frac) * x1,
            ]
    ...
    def getWidth(self, Ei_in=None, squared=False):
        ...
        if self.isFermi:
            ...
            wd = (
                (chop_times[1][1] - chop_times[1][0]) / 2.0 / 1.0e6,
                (chop_times[0][1] - chop_times[0][0]) / 2.0 / 1.0e6,
            )
    ...
    def _get_state(self, Ei_in=None):
        return hash(
            (
                self.variant,
                self.package,
                tuple(self.frequency),
                tuple(self.phase),
                Ei_in if Ei_in else self.ei,
                self.n_frame,
            )
        )
    ...
    def _MulpyRepDriver(self, Ei_in=None, calc_res=True):
        ...
        if calc_res:
            res_el, percent, chop_width, mod_width = MulpyRep.calcRes(
                Eis, chop_times, lastChopDist, self.chop_sam, self.sam_det
            )
    ...
    def package(self, value):
        ...
        if value not in self.packages.keys():
            ...
            if not ky:
                raise ValueError(
                    "Fermi package %s not recognised. Allowed values are: %s" % (value, ", ".join(self.packages.keys()))
                )
            else:
                value = ky[0]
        self._package = value
    ...
    def variant(self, value):
        ...
        if value not in self.variants.keys():
            ...
            if not ky:
                raise ValueError(
                    "Variant %s not recognised. Allowed values are: %s" % (value, ", ".join(self.variants.keys()))
                )
            else:
                value = ky[0]
...
class Instrument(object):
    ...
    def __init__(self, instrument, chopper=None, freq=None):
        ...
        for elem_nm, classref in zip(
            ["sample", "chopper_system", "moderator", "detector"], [Sample, ChopperSystem, Moderator, Detector]
        ):
            try:
        ...
        for prop in self.__child_properties:
            setattr(
                ...
                prop,
                property(
                    lambda obj, prop=prop, self=self: ChopperSystem.__dict__[prop].__get__(
    ...
    def calculate(cls, *args, **kwargs):
        ...
        return obj.getResolution(argdict["etrans"] if argdict["etrans"] else 0.0), obj.getFlux()
