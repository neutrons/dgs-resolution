...
from dgs_resolution.pychop import MulpyRep
from dgs_resolution.pychop.ISISFermi import ISISFermi
...
class ISISDisk:
    ...
    def __LETgetResolution(self, single_mode, Etrans=None, Ei_in=None):
        ...
        res_el, percent, chop_width, mod_width = MulpyRep.calcRes(
            Eis, chop_times, lastChopDist, self.chop_samp, self.samp_det
        )
        if single_mode:
        ...
        for ie in ie_list:
            ...
            for en in energy_transfer:
                ...
                if Ef > 0:
                    fac = (Ef / Eis[ie]) ** 1.5
                    chopRes = (2 * chop_width[ie] / t_mod_chop) * (
                        1 + ((self.chop_samp + lastChopDist) / self.samp_det) * fac
                    )
